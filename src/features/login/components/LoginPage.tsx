...
export default function LoginPage() {
  ...
  const { session, userRole, loading, authLoading, handleGoogleLogin } =
    useAuth();
  ...
  useEffect(() => {
    if (session && userRole === "admin") {
      ...
      setError("Your account does not have administrator permissions.");
    }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-neutral-bg flex flex-col items-center justify-center text-slate-500 space-y-4 animate-fade-in">
  ...
  return (
    <div className="min-h-screen bg-neutral-bg text-slate-800 flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans">
      ...
      <div className="relative z-10 w-full max-w-md bg-white border border-slate-200/80 rounded-3xl shadow-xl p-8 md:p-10 space-y-8 text-center animate-scale-up">
        ...
        <div className="flex flex-col items-center space-y-3">
          <div className="w-14 h-14 rounded-2xl bg-linear-to-tr from-cyan-400 to-lime-300 flex items-center justify-center font-extrabold text-2xl text-slate-950 shadow-md shadow-cyan-500/10 relative group">
            E
            <div className="absolute -top-1.5 -right-1.5 bg-cyan-600 text-white rounded-full p-1 border border-white shadow-sm">
              <ShieldCheck className="w-3.5 h-3.5" />
            </div>
          </div>
          <h1 className="text-2xl font-extrabold bg-linear-to-r from-cyan-600 to-teal-600 bg-clip-text text-transparent tracking-tight">
            Zóe Admin
          </h1>
        ...
        <div className="flex items-center justify-center space-x-1.5 text-[10px] text-slate-400 font-semibold border-t border-slate-100 pt-4">
          ...
          <span>Exclusive access for authorized administrators.</span>
        </div>
      </div>
