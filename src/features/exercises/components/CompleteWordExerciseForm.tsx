    ...
    <div className="w-full space-y-4">
      ...
      {items.map((item: any, itemIndex: number) => (
        <div key={itemIndex} className="mt-4 p-5 bg-slate-50/70 rounded-xl border border-slate-200/80 ">
          ...
          <div className="mt-4">
            <label className="text-sm font-medium text-slate-650 mb-2 block">Posibles respuestas</label>
            <div className="flex flex-wrap gap-2 mb-4">
              {item.possible_answers?.map((answer: string, idx: number) => (
                <div key={idx} className="bg-slate-100 px-3 py-1.5 rounded-full flex items-center gap-2 border border-slate-200 text-sm text-slate-700">
                  <span>{answer}</span>
                  <button
