...
export default function WriteWordExerciseForm({ order_index }: Props) {
  ...
  return (
    <div className="w-full space-y-4">
      ...
      {/* Game Mode Selector */}
      <div className="space-y-1.5 w-full bg-white p-4 rounded-xl border border-slate-200/60 shadow-sm">
        <label className="text-[10px] font-extrabold uppercase tracking-wider text-slate-450 block">
          Game Mode (Mobile App Behavior)
        </label>
        <select
          className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-xl p-3 placeholder-slate-400 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none transition-all duration-200 text-sm cursor-pointer"
